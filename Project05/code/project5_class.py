...
def color_sensor_data():
    print(
        ...
        "| Reflected:", robot.color_sensor.reflection(),
        "| Ambient:", robot.color_sensor.ambient()
    )

def distance_sensor_data():
    print("Distance (mm):", robot.distance_sensor.distance())
...
def timed_distance(time_ms,repeat_amt):
    ...
    for count in range(repeat_amt):
        print(robot.watch.time(), robot.distance_sensor.distance(), 'looooooong text', sep=",")
        wait(time_ms)
...
async def drive_motion():
    await robot.MoveForward_As(250)
    await wait(500)
    await robot.MoveBackward_As(250)
    await wait(500)

async def log_drive_data():
    print('Time', 'Distance', 'Speed', sep=",")
    while True:
        print(robot.watch.time(), robot.drive_base.distance(), robot.drive_base.state()[1], sep=",")
        await wait(10)
...
async def rotate_robot():
    ...
    await robot.TurnRight_As(360)
    await wait(200)

async def log_angle_distance():
    print("Angle,Distance")
    while True:
        angle = robot.drive_base.angle()
        distance = await robot.distance_sensor.distance()
...
async def drive_square(side_length=250):
    for _ in range(4):
        await robot.MoveForward_As(side_length)
        await robot.TurnRight_As(90)
        await wait(200)
...
async def log_data():
    ...
    while True:
        time_ms = robot.watch.time()
        distance = await robot.distance_sensor.distance()
        speed = robot.drive_base.state()[1]
        print(time_ms, distance, speed, sep=",")
        await wait(10)

# --- Run the experiment ---
async def run_experiment_51H():
    robot.drive_base.settings(straight_speed=300, straight_acceleration=200)
    robot.watch.reset()
    await multitask(
        drive_square(),
        log_data(),
...
def distance_loop_compare():
    ...
    print("Compare before loop:",
        robot.distance_sensor.distance() <= 500)

    while not robot.distance_sensor.distance() <= 500:
        wait(0)
...
def if_else():
    if robot.distance_sensor.distance() <= 500:
        robot.drive_base.stop()
    else:
        robot.drive_base.drive(200, 0)
    wait(0)
...
def distance_tone():
    ...
    while True:
        distance = robot.distance_sensor.distance()
        ...
        robot.hub.speaker.beep(freq, 50)
        wait(20)
...
def avoid_wall():
    while True:
        ...
        while not robot.distance_sensor.distance() <= 500:
            wait(0)
        if robot.distance_sensor.distance() <= 250:
            robot.MoveBackward(250)
            robot.TurnLeft(120)
        else:
            robot.MoveBackward(250)
...
def avoid_wall_alt():
    while True:
        ...
        current_distance = robot.distance_sensor.distance()
...
def bumper_turn():
    while True:
        robot.drive_base.drive(200, 0)
        while not (Button.LEFT in robot.hub.buttons.pressed() or Button.RIGHT in robot.hub.buttons.pressed()):
            wait(0)
...
def drive_until_condition():
    ...
    while not (
        robot.drive_base.distance() >= 500 or
        robot.distance_sensor.distance() <= 200
    ):
        wait(10)
...
def wave_turn():
    while True:
        ...
        while robot.distance_sensor.distance() > 150:
            wait(10)

        while robot.distance_sensor.distance() < 400:
            wait(10)
