...
def square_curved_corners(W, R):
    ...
    for _ in range(4):
        robot.MoveForward(straight_length)
        robot.drive_base.curve(R, 90)  # radius R, 90° turn
...
def square_beep(side_length):
    while True:
        ...
        robot.hub.speaker.beep(1200, 200)
...
def slow_mode():
    robot.drive_base.settings(100, 100, 90, 90)

def fast_mode():
    robot.drive_base.settings(500, 500, 360, 360)
...
def motors_run_angle(speed, rotation_angle):
    robot.left_motor.run_angle(speed, rotation_angle, wait=False)
    robot.right_motor.run_angle(speed, rotation_angle, wait=True)
...
async def blink():
    while blinking["active"]:
        robot.hub.light.on(blink_color["value"])
        await wait(200)
        robot.hub.light.off()
        await wait(200)

async def play_melody():
    await robot.hub.speaker.play_notes(
        [
            "C3/4", "C3/4",
            "G3/4", "G3/4",
            "A3/4", "A3/4",
...
async def drive_square_dance(side_length):
    for i in range(4):
        await multitask(
            robot.MoveForward_As(side_length),
            change_color(Color.BLUE)
        )
        await multitask(
            robot.TurnRight_As(90),
...
async def run_multitask():
    await multitask(
        robot.left_motor.run_angle(500, 360),
        robot.hub.speaker.beep(500, 100),
        combined_task(),
    )
...
async def drive_back(distance):
    await robot.MoveBackward_As(distance)
...
async def subtask():
    for count in range(5):
        ...
        await multitask(
            robot.left_motor.run_angle(500, 360),
            robot.hub.speaker.beep(500, 700),
        )
